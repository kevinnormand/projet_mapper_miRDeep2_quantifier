...
echo "nom du fichier des resultats de mirdeep2 : ${1}">>log.txt	#indicate the name of file used in first parameter in a file called log
echo "espece recherchee : ${2}">>log.txt	#indicate the word used in parameter for specie researched in the file called log
echo "score selectionné pour la recherche de new mirna : ${3}">>log.txt	#indicate the score use in parameter to create new_mirna.csv in a file called log
echo "option choisie pour le filtre sur les randfold significatif : ${4}">>log.txt	#indicate the option randfold chosen by user in a file called log
echo "option choisie pour le filtre sur les exemple de mirna avec la meme seed region : ${5}">>log.txt	#indicate the option example with mirna in miRBase chosen in a file called log
...
cmd2="awk -F '\t' '{if(${condition}) {print \"${2}-\" \$1 \"\t\" \$0}else if(\$9 ==\"\"){exit}else{next}}' fichier_apres_tail > new_mirna.csv;" 	
#command to recuperate lines of "fichier_apres_tail" which respect "condition" in "new_mirna.csv"

echo "execution de la commande suivante :">>log.txt
echo $cmd>>log.txt	#display the command used in a file called log
echo "execution de la commande suivante :">>log.txt
echo $cmd2>>log.txt #display the command used in a file called log

(eval "$cmd") 2>>log.txt	#execute the command
(eval "$cmd2") 2>>log.txt	#execute the command

echo "new_mirna.csv OK">>log.txt	#display "new_mirna.csv OK" to show the end of the creation of new_mirna.csv in a file called log
...
cmd3="awk -F '\t' '{if(\$1 !=\"${2}-provisional id\"){print \">\" \$1 \"_mt\n\" \$15 }}' new_mirna.csv > new_mirna.fa"	
#command to recuperate mirna where "_mt" is present in the identifier of file new_mirna.csv in the file new_mirna.fa
cmd4="awk -F '\t' '{if(\$1 !=\"${2}-provisional id\"){print \">\" \$1 \"_st\n\" \$16 }}' new_mirna.csv >> new_mirna.fa"	
#command to recuperate mirna where "_st" is present in the identifier of file new_mirna.csv in the file new_mirna.fa
...
cmd5="awk -F '\t' '{if(\$1 !=\"${2}-provisional id\"){print \">\" \$1 \"\n\" \$17 }}' new_mirna.csv > new_hairpin.fa"	
#command to recuperate precursors'mirna of file new_mirna.csv in file new_hairpin.fa

echo "execution de la commande suivante : ">>log.txt
echo $cmd5>>log.txt	#display the command used in the file called log

(eval "$cmd5") 2>>log.txt	#execut the command 5
...
cmd6="awk -F '\t' '{if(\$1 !=\"${2}-provisional id\"){print \$1 \"\t\" \$18}}' new_mirna.csv | perl -p -e 's/:|\.\./\t/g' | awk -F '\t' '{print \$2 \"\tmiRDeep2\tnew_predicted_precursor\t\" \$3 \"\t\" \$4 \"\t.\t\" \$5 \"\t.\t\" \$1}' > news_predicted_precursors.gff"
#command to recuperate positions of precursors on the genome of file new_mirna.csv in file news_predicted_precursors.gff
...
#command to recuperate mirna know of file of mirdeep2 in the file know_${2}_mirna.csv, cat is not necessarly but used to have the standard output of file for awk
...
cmd8="awk -F '\t' '{if(\$1 !=\"${2}-tag id\"){print \">\" \$1 \"_mt\n\" \$15 }}' know_${2}_mirna.csv > predicted_know_mirna.fa"		
#command to recuperate mirna where "_mt" is present in the identifier of file know_${2}_mirna.csv in the file predicted_know_mirna.fa
cmd9="awk -F '\t' '{if(\$1 !=\"${2}-tag id\"){print \">\" \$1 \"_st\n\" \$16 }}' know_${2}_mirna.csv >> predicted_know_mirna.fa"	
#command to recuperate mirna where "_st" is present in the identifier of file know_${2}_mirna.csv in the file predicted_know_mirna.fa
...
echo "creation de predicted_know_hairpin.fa">>log.txt	#indicate the beginning of the creation of predicted_know_hairpin.fa in the file called log

cmd10="awk -F '\t' '{if(\$1 !=\"${2}-tag id\"){print \">\" \$1 \"\n\" \$17 }}' know_${2}_mirna.csv > predicted_know_hairpin.fa"	
#command to recuperate precursors of file know_${2}_mirna.csv in the file predicted_know_hairpin.fa
...
echo "predicted_know_hairpin.fa OK">>log.txt	#display "predicted_know_hairpin.fa OK" to show the end of the creation of predicted_know_hairpin.fa in the file called log
echo "">>log.txt	#display a blank space in the file called log
...
cmd12="modification_identifier.py ${7} mature_sans_espace.fa"	#keep informations before the first space
...
###############creation identified_espece_mirna.id#####################
echo "creation du fichier identified_espece_mirna.id">>log.txt	#indicate the beginning of the creation of identified_espece_mirna.id in the file called log

cmd13="cut -f 11 know_${2}_mirna.csv | grep -v miRBase | sort -u > identified_${2}_mirna.id"	
#command to recuperate mirna already existing in miRBase of file know_${2}_mirna.csv in the file identified_${2}_mirna.id
...
echo "identified_espece_mirna.id OK">>log.txt	#display "identified_espece_mirna.id OK" to show the end of the creation of identified_espece_mirna.id in the file called log
echo "">>log.txt	#display a blank space in the file called log

################creation identified_espece_mira.pre.id##################
echo "creation du fichier identified_espece_mirna.pre.id">>log.txt	#indicate the beginning of the creation of identified_espece_mirna.pre.id in the file called log
...
echo "execution de la commande suivante : ">>log.txt
echo $cmd14>>log.txt	#display the command used in the file called log

(eval "$cmd14") 2>>log.txt	#execute the command 14

echo "identified_espece_mirna.pre.id OK">>log.txt	#display "identified_espece_mirna.pre.id OK" to show the end of the creation of identified_espece_mirna.pre.id in the file called log
echo "">>log.txt	#display a blank space in the file called log

###############creation no_identified_espece_mirna_pre.fasta######################
echo "creation du fichier no_identified_espece_mirna_pre.fasta">>log.txt #indicate the beginning of the creation of no_identified_espece_mirna_pre.fasta in the file called log

cmd15="extract_seqs_by_ids_modifier.pl --fasta_in ${8} --ids_file identified_${2}_mirna.pre.id --file_out no_identified_${2}_mirna.pre.fasta --precursor_shape yes --inverse yes"	
#command to recuperate identifiers without word of identified_${2}_mirna.pre.id in no_identified_${2}_mirna.pre.fasta

echo "execution de la commande suivante : ">>log.txt
echo $cmd15>>log.txt	#display the command used in the file called log

(eval "$cmd15") 2>>log.txt	#execute the command 15

echo "no_identified_espece_mirna_pre.fasta OK">>log.txt	#display "no_identified_espece_mirna_pre.fasta OK" to show the end of the creation of no_identified_espece_mirna_pre.fasta in the file called log
echo "">>log.txt	#display a blank space in the file called log
...
cmd16="cat new_mirna.fa predicted_know_mirna.fa mature_sans_espace.fa > mature_and_new.fa"	#command to concatenate files new_mirna.fa, predicted_know_mirna.fa and mature_sans_espace.fa in mature_and_new.fa

echo "execution de la commande suivante : ">>log.txt
echo $cmd16>>log.txt	#display the command used in the file called log

(eval "$cmd16") 2>>log.txt	#execute the command 16
...
cmd17="cat new_hairpin.fa predicted_know_hairpin.fa no_identified_${2}_mirna.pre.fasta > hairpin_espece_and_new.fa"	
...
echo "execution de la commande suivante : ">>log.txt
echo $cmd17>>log.txt	#display the command used in the file called log

(eval "$cmd17") 2>>log.txt	#execute the command 17
